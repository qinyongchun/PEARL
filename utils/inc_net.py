...
def get_backbone(args, pretrained=False):
    ...
    # VPT
        ...
        ffn_num = args["ffn_num"]
        if args["model_name"] == "adam_spa" or args["model_name"] == "spa":
            from backbone import vision_transformer_spa
            from easydict import EasyDict
            tuning_config = EasyDict(
                # AdaptFormer
                ffn_adapt=True,
            ...
            if name == "pretrained_vit_b16_224_spa":
                model = vision_transformer_spa.vit_base_patch16_224_spa(num_classes=0,
                    global_pool=False, drop_path_rate=0.0, tuning_config=tuning_config,
                    nb_tasks=args["nb_tasks"], prompt_param=args["prompt_param"], 
                    prompt_width=args["prompt_width"])
                model.out_dim=768
            elif name == "pretrained_vit_b16_224_in21k_spa":
                model = vision_transformer_spa.vit_base_patch16_224_in21k_spa(num_classes=0,
                    global_pool=False, drop_path_rate=0.0, tuning_config=tuning_config,
                    nb_tasks=args["nb_tasks"], prompt_param=args["prompt_param"],
                    prompt_width=args["prompt_width"])
                model.out_dim=768
    ...
    # TSP
        ...
        if args["model_name"] == "tsp":
            ...
            from backbone import vision_transformer_tsp
            model = vision_transformer_tsp.vit_base_patch16_224_tsp_2(
...
class SimpleVitNet(BaseNet):
    ...
    def forward(self, x):
        ...
        out.update({"features": x})
        return out
class SimpleVitNet_SPA(BaseNet):
    def __init__(self, args, pretrained):
        super().__init__(args, pretrained)
        # for RanPAC
        self.W_rand = None
    ...
    def forward(self, x,train=False,reset_fix=False,old=False):
        x = self.backbone(x,train=train,sigma=self.sigma,reset_fix=reset_fix,old=old)
        # print(x.shape)
        if self.W_rand is not None:
            x = torch.nn.functional.relu(x @ self.W_rand)
        out = self.fc(x)
    ...
    def extract_vector(self, x,cur_task):
        with torch.no_grad():
            if self.original_backbone is not None:
                ...
                cls_features = None
        return self.backbone(x, task_id=cur_task, cls_features=cls_features, train=False)['pre_logits']
        # return self.backbone(x)
...
class CodaPromptVitNet(nn.Module):
    def __init__(self, args, pretrained):
        ...
        self.prompt = CodaPrompt(768, args["nb_tasks"], args["prompt_param"],lamda=args["lamda"])
...
# tsp
class TSPVitNet(nn.Module):
    def __init__(self, args, pretrained):
        super(TSPVitNet, self).__init__()
        self.args = args
        self.backbone = get_backbone(args, pretrained)
        # self.fc = CosineLinear(768, args["nb_classes"])
        self.prompt = TSPrompt(768, args["nb_tasks"], args["prompt_param"], args["prompt_width"])
    ...
    def update_fc(self, nb_classes, nextperiod_initialization=None):
        ...
        if hasattr(self,'fc'):
            ...
            del self.fc
        self.fc = fc
    def update_fc_ranpac(self, _cur_task, nb_classes, nextperiod_initialization=None):
        if self.RP_dim is not None:
            feature_dim = self.RP_dim
        else:
            feature_dim = self.feature_dim
    ...
    # pen: get penultimate features  
    def forward(self, x, pen=False, train=False,old=False,sigma=0,reset_fix=False):
            if self.prompt is not None:
                out, prompt_loss = self.backbone(x, prompt=None, old = old,sigma=sigma, train=train,reset_fix=reset_fix)
                out = out[:,0,:]
            else:
                out, _ = self.backbone(x)
                out = out[:,0,:]
            ...
            if not pen:
                if self.W_rand is not None:
                    out = torch.nn.functional.relu(out @ self.W_rand)
                out = self.fc(out)
            if self.prompt is not None and train:
                return out, prompt_loss
            else:
        ...
        # else:
        #     if self.prompt is not None:
        #         out, prompt_loss = self.backbone(x, prompt=self.prompt, old = old, train=train,sigma=sigma,reset_fix=reset_fix)
        #         out = out[:,0,:]
        #     else:
        #         out, _ = self.backbone(x)
        #         out = out[:,0,:]
