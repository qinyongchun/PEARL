...
import copy
from backbone.prompt_spa import Attention,Block
...
class CodaPrompt(nn.Module):
    def __init__(self, emb_d, n_tasks, prompt_param, key_dim=768,lamda=0.000):
        super().__init__()
        self.task_count = 0
        self.emb_d = emb_d
        self.key_d = key_dim
    ...
    def forward(self, x_querry, l, x_block, train=False):
        ...
        if l in self.e_layers:
            ...
            # lamda = 0.01
            # p = self.blocks(p)*self.lamda+p*(1-self.lamda)
...
class EPrompt(nn.Module):
    def __init__(self, length=5, embed_dim=768, embedding_key='mean', prompt_init='uniform', prompt_pool=False, 
                 ...
                 num_layers=1, use_prefix_tune_for_e_prompt=False, num_heads=-1, same_key_value=False,lamda=0.00):
        super().__init__()
        ...
        self.blocks = nn.Sequential(*[
            Block(
                dim=768, num_heads=2, mlp_ratio=1, qkv_bias=False,
                drop=0, attn_drop=0
            )
            for i in range(4)])
    ...
    def forward(self, x_embed, prompt_mask=None, cls_features=None):
        ...
        if self.prompt_pool:
            ...
            if self.use_prefix_tune_for_e_prompt:
                ...
                # print(batched_prompt_raw[0,0,0])
                # print(batched_prompt_raw.shape)
                batch_size = batched_prompt_raw.size(2)
                original_shape = batched_prompt_raw.shape
                batched_prompt_pe = batched_prompt_raw.permute(2, 0, 1, 3, 4, 5, 6)
                new_shape = (batch_size, -1, 768)
                batched_prompt_pe = batched_prompt_pe.reshape(*new_shape)
                batched_prompt_pe = self.blocks(batched_prompt_pe)             
                batched_prompt_pe = batched_prompt_pe.reshape(batch_size, 3, 2, 1, 5, 12, 64)
                batched_prompt_pe = batched_prompt_pe.permute(1, 2, 0, 3, 4, 5, 6) 
                # print(batched_prompt_raw.shape)
                # print(batched_prompt_pe[0,0,0])
                # print(batched_prompt_raw[0,0,0])
                # print("11111111111111111111")
                batched_prompt_raw = batched_prompt_raw*(1-self.lamda)+batched_prompt_pe*self.lamda
...
class Prompt(nn.Module):
    def __init__(self, length=5, embed_dim=768, embedding_key='mean', prompt_init='uniform', prompt_pool=False, 
                 prompt_key=False, pool_size=None, top_k=None, batchwise_prompt=False, prompt_key_init='uniform',lamda=0.000001):
        super().__init__()
    ...
    def forward(self, x_embed, prompt_mask=None, cls_features=None):
        ...
        if self.prompt_pool:
            ...
            batched_prompt = batched_prompt*(1-self.lamda)+self.blocks(batched_prompt)*self.lamda
