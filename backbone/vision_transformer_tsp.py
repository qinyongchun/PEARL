...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # print(config.ffn_adapt)
        self.config = config
        if self.config.ffn_adapt:
            self.adaptmlp = Adapter(config, dropout=0.1, bottleneck=config.ffn_num,
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 ckpt_layer=0, tuning_config=None):
        """
        Args:
        ...
        #         drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, config=self.tuning_config,
        #         )
        ...
        if self.tuning_config.vpt_on:
            assert self.tuning_config.vpt_num > 0, self.tuning_config.vpt_num
            # properly registered
    ...
    def forward(self, x, register_blk=-1, sigma=0,prompt=None, old = False, train=False, task_id=None,reset_fix=False):
        B = x.shape[0]
        ...
        for i,blk in enumerate(self.blocks):
            ...
            if prompt is not None:
                if train:
                    p_list, loss, x = prompt.forward(i, x, sigma=sigma, old = old, train=True,reset_fix=reset_fix)
                    prompt_loss += loss
                else:
                    p_list, _, x = prompt.forward(i, x, sigma=sigma, old = old, train=False,reset_fix=reset_fix)
            else:
                p_list = None
        ...
        # if prompt is not None:
        #     pt = int(prompt.e_pool_size / (prompt.n_tasks))
        #     s = int(prompt.task_count * pt)
        #     f = int((prompt.task_count + 1) * pt)
        #     now_logit = x[:,0:s]

        #     #计算now_logit和old_logits之间的KL散度
        #     if old_logits is not None:
        #         # old_logits = old_logits[:,0:s]
        #         print(x.shape)
        #         print(now_logit.shape)
        #         print(old_logits.shape)

        #         old_logits = F.log_softmax(old_logits, dim=1)
        #         now_logit = F.log_softmax(now_logit, dim=1)
        #         kl = F.kl_div(now_logit, old_logits, reduction='batchmean')
        ...
        #     if prompt is not None:
        #         if train:
        #             p_list, loss, x = prompt.forward( 12, x, sigma, old = old, train=True)
        #         else:
        #             p_list, loss, x = prompt.forward( 12, x, sigma, old = old, train=False)
        #     else:
        #             p_list = None
        #     x = blk(x, register_blk==i, prompt=p_list)
        # x = self.norm(x)
...
def vit_base_patch16_224_tsp_2(pretrained=False, **kwargs):
...
@register_model
def vit_tiny_patch16_224_tsp(pretrained=False, **kwargs):
    """ ViT-Tiny (Vit-Ti/16)
    """
...
@register_model
def vit_tiny_patch16_384_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_small_patch32_224_tsp(pretrained=False, **kwargs):
    """ ViT-Small (ViT-S/32)
    """
...
@register_model
def vit_small_patch32_384_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_small_patch16_224_tsp(pretrained=False, **kwargs):
    """ ViT-Small (ViT-S/16)
...
@register_model
def vit_small_patch16_384_tsp(pretrained=False, **kwargs):
    """ ViT-Small (ViT-S/16)
    NOTE I've replaced my previous 'small' model definition and weights with the small variant from the DeiT paper
    """
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, **kwargs)
...
@register_model
def vit_base_patch32_224_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch32_384_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch16_224_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch16_384_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch8_224_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_large_patch32_224_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_large_patch32_384_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_large_patch16_224_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_large_patch16_384_tsp(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 384x384, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, **kwargs)
...
@register_model
def vit_large_patch14_224_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_huge_patch14_224_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_giant_patch14_224_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_gigantic_patch14_224_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_tiny_patch16_224_in21k_tsp(pretrained=False, **kwargs):
    """ ViT-Tiny (Vit-Ti/16).
...
@register_model
def vit_small_patch32_224_in21k_tsp(pretrained=False, **kwargs):
    """ ViT-Small (ViT-S/16)
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
...
@register_model
def vit_small_patch16_224_in21k_tsp(pretrained=False, **kwargs):
    """ ViT-Small (ViT-S/16)
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
...
@register_model
def vit_base_patch32_224_in21k_tsp(pretrained=False, **kwargs):
    """ ViT-Base model (ViT-B/32) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
...
@register_model
def vit_base_patch16_224_in21k_tsp(pretrained=False, **kwargs):
    """ ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
...
@register_model
def vit_base_patch8_224_in21k_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_large_patch32_224_in21k_tsp(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/32) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
...
@register_model
def vit_large_patch16_224_in21k_tsp(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
...
@register_model
def vit_huge_patch14_224_in21k_tsp(pretrained=False, **kwargs):
    """ ViT-Huge model (ViT-H/14) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has a representation layer but the 21k classifier head is zero'd out in original weights
    """
...
@register_model
def vit_base_patch16_224_sam_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch32_224_sam_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_small_patch16_224_dino_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_small_patch8_224_dino_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch16_224_dino_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch8_224_dino_tsp(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch16_224_miil_in21k_tsp(pretrained=False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
...
@register_model
def vit_base_patch16_224_miil_tsp(pretrained=False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    Weights taken from: https://github.com/Alibaba-MIIL/ImageNet21K
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, qkv_bias=False, **kwargs)
...
@register_model
def vit_base_patch32_plus_256_tsp(pretrained=False, **kwargs):
    """ ViT-Base (ViT-B/32+)
    """
...
@register_model
def vit_base_patch16_plus_240_tsp(pretrained=False, **kwargs):
    """ ViT-Base (ViT-B/16+)
    """
...
@register_model
def vit_small_patch16_36x1_224_tsp(pretrained=False, **kwargs):
