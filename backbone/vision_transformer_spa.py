...
class Attention(nn.Module):
    ...
    def forward(self, x, prompt=None):
        ...
        if prompt is not None:
            ...
            k = k.view(B*self.num_heads, N+prompt_length, self.head_dim)
            v = v.view(B*self.num_heads, N+prompt_length, self.head_dim)
...
from backbone.prompt_spa import SPAprompt

class VisionTransformer(nn.Module):
    """ Vision Transformer with support for global average pooling
    """
    def __init__(self,nb_tasks,prompt_param,prompt_width, global_pool=False, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.prompt = SPAprompt(768, nb_tasks,prompt_param,prompt_width)
    ...
    def forward_features(self, x,train,sigma,old,reset_fix):
        B = x.shape[0]
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if self.prompt is not None:
                if train:
                    p_list, loss, x = self.prompt.forward(idx, x, sigma=sigma, old = old, train=True,reset_fix=reset_fix)
                    # prompt_loss += loss
                else:
                    p_list, _, x = self.prompt.forward(idx, x, sigma=sigma, old = old, train=False,reset_fix=reset_fix)
            else:
                p_list = None
    ...
    def forward(self, x,train=True,sigma=0,old=False,reset_fix=False):
        # print(11111111111111111111111111111111111111111111111111111)
        x = self.forward_features(x,train,sigma,old,reset_fix)
...
def vit_base_patch16_224_spa(pretrained=False, **kwargs):
...
def vit_base_patch16_224_in21k_spa(pretrained=False, **kwargs):
