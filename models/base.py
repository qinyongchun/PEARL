...
class BaseLearner(object):
    ...
    def _eval_cnn(self, loader):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                # print('sigma:',self._network.sigma)
            predicts = torch.topk(
    ...
    def _eval_proto(self, loader,prototypes):
        self._network.eval()
        vectors, y_true = self._extract_vectors(loader)
        vectors = (vectors.T / (np.linalg.norm(vectors.T, axis=0) + EPSILON)).T
        # print('vectors shape:',vectors)
        # print('proto shape:',prototypes)

        pronorm = (prototypes.T / (torch.linalg.norm(prototypes.T, axis=0) + EPSILON)).T

        dists = cdist(tensor2numpy(pronorm.cpu()), vectors, "sqeuclidean")  # [nb_classes, N]
        # print('dists shape:',dists.shape)
        scores = dists.T  # [N, nb_classes], choose the one with the smallest distance
        # print(np.argsort(scores, axis=1)[:, : self.topk])
        return np.argsort(scores, axis=1)[:, : self.topk], y_true  # [N, topk]
            ...
            for _, _inputs, _targets in loader:
                ...
                if isinstance(self._network, nn.DataParallel):
                    _vectors = tensor2numpy(
                        self._network.module.extract_vector(_inputs.to(self._device),cur_task=self._cur_task)
                    )
                else:
                    _vectors = tensor2numpy(
                        self._network.extract_vector(_inputs.to(self._device),cur_task=self._cur_task)
                    )
    ...
    def _extract_vectors_torch(self, loader):
        #功能与_extract_vectors一致，但是输出为torch.tensor，cuda0
        self._network.eval()
        vectors, targets = [], []
        with torch.no_grad():
            for _, _inputs, _targets in loader:
